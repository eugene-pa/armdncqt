...
//   а именно: с использованием функции GetCh(int ms) - получить символ в течении ограниченного времени
...
//   - передачу данных
// Важно:
// 1.Попытка реализовать прием данных без использования сообщений QT в потоке std::thread
//   путем использования waitForReadyRead приводила к задержкам в обработке двнных:
...
bool RsAsinc::Send (const char *p, int length)
{
#ifdef CONSOLAPP
    if (serial.write(p, length) <= 0)
        return false;
    return serial.waitForBytesWritten(-1);         // только для консольных приложений!
#else
    return length == serial.write(p, length);
#endif
}
