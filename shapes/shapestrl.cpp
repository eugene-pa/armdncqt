...
void ShapeStrl::Parse(QString& src)
{
    ...
    if (lexems.length() >= 14)
    {
        ...
        boundRect = QRect(x1-7,y1-15,rect.width()+14, rect.height()+14);
...
void ShapeStrl::Draw(QPainter* painter)
{
    ...
    // 3. окантовки
    if (sprStrl && sprStrl->Locked())                       // законсервирована
        drawRect(painter, *PenIzsRound);

//    if (StsAct[StsMu   ])                                                           // МУ
//        drawRect(dc, PenMuRect, 1.0);

//    if (StsAct[StsIzs])                                                             // искусственно замкнута
//        drawRect(dc, PenIzsRound, 0.7);

//    // можно мигать как сейчас. например, менять прозрачность
//    if (StsAct[StsOtu])
//        drawRect(dc, PenOtuRect, GlobalPulsingTrigger ? 1.0 : 0.7);
...
//    dc.DrawRectangle(null, pen, new Rect(Area.X - dxy / 2, Area.Y - dxy / 2, Width + dxy, Height + dxy));
//    if (transparent)
//        dc.Pop();
}
